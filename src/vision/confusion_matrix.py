...
def generate_confusion_data(
    ...
    with torch.no_grad():  # Disable gradient computation for inference
        for i, (images, labels) in enumerate(loader):
            # for each batch, [batch,C,H,W], batch indicates how many pictures are processed together.
            if cuda:
                images = images.cuda()
                labels = labels.cuda()
            ...
            # Update the preds and targets arrays

            start_index = i * batch_size # each parts have batch size
            end_index = start_index + labels.size(0) 

            preds[start_index:end_index] = predicted.cpu().numpy()
            targets[start_index:end_index] = labels.cpu().numpy() # total_num_of_pic/batch * class_num * batch
    
    class_labels = list(label_to_idx.keys()) # convert from dictionary data into list data
...
def generate_confusion_matrix(
    ...
    for target, prediction in zip(targets, preds):
        ...
        confusion_matrix[target, prediction] += 1 # counts, the target is row coord, and prediction is column coord
...
def generate_accuracy_table(
    ...
    # Student code begins here
    ##########################################################################
    # N is sample time, k is class_num
