...
def test_compute_ransac_iterations():
    data_set = [
        (0.99, 1, 0.99, 1),
        (0.99, 10, 0.9, 11),
        (0.9, 15, 0.5, 75450),
    ...
    for prob_success, sample_size, ind_prob, num_samples in data_set:
        S = compute_ransac_iterations(prob_success, sample_size, ind_prob)
...
def test_ransac_fundamental():
    ...
    F, _, _ = ransac_fundamental(points_2d_pic_a, points_2d_pic_b)
    expected_F = np.array(
        [
